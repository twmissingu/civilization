...
import type { GameState, CityState, UnitState, ProductionItem } from './types';
...
/** 每回合城市结算：食物/人口/文化扩张/生产 */
export function settleCity(state: GameState, city: CityState): void {
  ...
  if (city.queue.length > 0) {
    ...
    if (item.progress >= cost) {
      item.progress -= cost;
      completeProduction(state, city, item);
      city.queue.shift();
    }
  }
  // 住房重算
...
function completeProduction(state: GameState, city: CityState, item: ProductionItem): void {
  const owner = state.players.find((p) => p.id === city.ownerId)!;
  if (item.kind === 'unit') {
    const u = UNITS[item.id];
    if (!u) return;
